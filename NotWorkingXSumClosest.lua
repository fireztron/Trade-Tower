...
--[[
local function solution(arr, x, n)
    ...
    for i = 1, #arr, 1 do
        for j = i + 1, #arr, 1 do
            for k = j + 1, #arr, 1 do
                ...
                --if math.abs(x - closestSum) > math.abs(x - sum) and sum <= x then
...
local arr = {-1, 2, 1, -4}
local closestSum, elems = solution(arr, 1)
...
local function solution(arr, x, target)
    local i, j, k, p
    local diff = math.huge
    local length = getSize(arr)
    local tmpsum;
    local sum
    local start, end1
    local res = {}
    ...
    if x > 2 then
        ...
        for j = x - 3, 0, -1 do
            ...
            while (k < (length - 2)) do
                ...
                tmpsum = 0
                p = j + 1
                while ((p < x - 2) and (index[p - 1] < length - 2)) do
                    index[p] = index[p-1] + 1
                    for aa, bb in pairs(index) do
                        print("AH!")
                        print(aa, bb)
                    end
                    tmpsum = tmpsum + getPrice(arr[index[p]])
                    p = p + 1
                end
                ...
                local tmplist = {}
                for p = 0, x - 3, 1 do
                    table.insert(tmplist, arr[index[p]])
                end

                start = index[x - 3] + 1
                end1 = length - 1
                sum = tmpsum + getPrice(arr[start]) + getPrice(arr[end1])

                while (start < end1) do
                    sum = tmpsum + getPrice(arr[start]) + getPrice(arr[end1])
                    if (math.abs(sum - target) < math.abs(diff)) then
                        diff = math.abs(sum-target)
                        sum = tmpsum + getPrice(arr[start]) + getPrice(arr[end1])
                        local tmp = {}
                        table.insert(tmp, arr[start])
                        table.insert(tmp, arr[end1])
                        res = {}
                        table.insert(res, tmp)
                    elseif (math.abs(sum - target) == math.abs(diff)) then
                        diff = math.abs(sum - target)
                        local tmp = {}
                        table.insert(tmp, arr[start])
                        table.insert(tmp, arr[end1])
...
print(TableToString(solution(arr, 4, 1)))
